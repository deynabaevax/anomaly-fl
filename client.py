...
class AnomalyClient(fl.client.NumPyClient):
    ...
    # Params sent from the server to the client
    def fit(
    ...
    # The server has aggregated the weights, so it'll send the agg weights to every client to evaluate the model to their own data
    def evaluate(
        self, parameters: List[np.ndarray], config: Dict[str, str]
...
# Aggregates metrics from the CSV files generated by all clients into a single df
...
# Show the average training and testing loss per round for all clients
